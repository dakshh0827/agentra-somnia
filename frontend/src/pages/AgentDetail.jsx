...
function DbPurchasePanel({ agent, onSuccess }) {
  ...
  const handlePurchase = async () => {
    ...
    try { await agentsAPI.purchaseAccess(agent.agentId, purchaseType === 'lifetime', null); onSuccess() }
    catch (e) { setError(e?.response?.data?.error || 'Purchase failed') }
    finally { setIsPurchasing(false) }
  }
  return <PurchasePanelUI purchaseType={purchaseType} setPurchaseType={setPurchaseType} monthlyEth={monthlyEth} lifetimeEth={lifetimeEth} multiplier={multiplier} onPurchase={handlePurchase} isPurchasing={isPurchasing} error={error} isBlockchain={false} />
}
...
function BlockchainPurchasePanel({ agent, onSuccess }) {
  ...
  return <PurchasePanelUI purchaseType={purchaseType} setPurchaseType={setPurchaseType} monthlyEth={monthlyEth} lifetimeEth={lifetimeEth} multiplier={Number(multiplier)} onPurchase={handlePurchase} isPurchasing={isPurchasing} error={error} isBlockchain={true} />
}
